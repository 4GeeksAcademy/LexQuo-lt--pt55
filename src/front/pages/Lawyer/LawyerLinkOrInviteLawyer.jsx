...
export const LawyerLinkOrInviteLawyer = () => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      if (resp.status === 404) { setFoundLawyer(null); setNotFound(true); return; }

      const data = await resp.json().catch(() => ({}));
      if (!resp.ok) throw new Error(data.error || `HTTP ${resp.status}`);

      if (data?.found && data?.lawyer) { setFoundLawyer(data.lawyer); setNotFound(false); }
      else { setFoundLawyer(null); setNotFound(true); }
    } catch (err) {
      ...
      <div className="container add-page">
        <div className="row">
          <div className="col-12 col-lg-8">
            {/* Header */}
            {/* Header (estética alineada) */}
            <div className="d-flex justify-content-between align-items-center mb-4">
              <h1 className="display-5 fw-bold mb-0">Link or Create Lawyer</h1>
            ...
            {preselectedCourtfileId && (
              <span className="badge bg-dark mt-2 mb-3">
